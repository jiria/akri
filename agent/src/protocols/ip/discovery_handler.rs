...
use failure::Error;
//use rust_scan::{scanner::Scanner, port_strategy::PortStrategy};
use std::{collections::HashMap};//, time::Duration};
use port_scanner::scan_ports_addrs;

/// `IpDiscoveryHandler` discovers the ip instances as described by the filters `discover_handler_config.ip_addresses`,
/// `discover_handler_config.mac_addresses`, and `discover_handler_config.scopes`.
/// The instances it discovers are always shared.
#[derive(Debug)]
pub struct IpDiscoveryHandler {
    discovery_handler_config: IpDiscoveryHandlerConfig,
}

impl IpDiscoveryHandler {
    pub fn new(discovery_handler_config: &IpDiscoveryHandlerConfig) -> Self {
        IpDiscoveryHandler {
            discovery_handler_config: discovery_handler_config.clone(),
        }
    }

    // TODO use this to filter based on macs if specified
    ...
    //         let (ip_address, mac_address) = match ip_query
    ...
    //         if IpDiscoveryHandler::execute_filter(
    ...
    //         let device_scopes = match ip_query.get_device_scopes(&device_service_url).await {
    //             Ok(scopes) => scopes,
    //             Err(e) => {
    ...
    //         properties.insert(
    //             IP_DEVICE_SERVICE_URL_LABEL_ID.to_string(),
    //             device_service_url.to_string(),
    //         );
    //         properties.insert(IP_DEVICE_IP_ADDRESS_LABEL_ID.into(), ip_address);
    //         properties.insert(IP_DEVICE_MAC_ADDRESS_LABEL_ID.into(), mac_address);
...
#[async_trait]
impl DiscoveryHandler for IpDiscoveryHandler {
    async fn discover(&self) -> Result<Vec<DiscoveryResult>, failure::Error> {
        // let ip_query = IpQueryImpl {};

        info!("discover - filters:{:?}", &self.discovery_handler_config,);

        // TODO handle net masks, possibly already covered
        //let mut result = Vec::new(); // TODO expression?
        let result = scan_ports_addrs(self.discovery_handler_config.ip_addresses.iter().map(
        ...
        // TODO use MAC for filtering

        // let discovered_ip_cameras = util::simple_ip_discover(Duration::from_secs(
        //     self.discovery_handler_config.discovery_timeout_seconds as u64,
        // ))
        // .await?;
        // info!("discover - discovered:{:?}", &discovered_ip_cameras,);
        // let filtered_ip_cameras = self
        //     .apply_filters(discovered_ip_cameras, &ip_query)
        //     .await;
        // info!("discover - filtered:{:?}", &filtered_ip_cameras);
        // filtered_ip_cameras
...
// mod tests {
//     use super::*;
//     use akri_shared::ip::device_info::test_ip::MockIpQueryImpl;
...
//         let mut mock = MockIpQueryImpl::new();
//         configure_scenario(
//             &mut mock,
//             Some(IpAndMac {
//                 mock_uri: "device_uri",
...
//         let ip = IpDiscoveryHandler::new(&IpDiscoveryHandlerConfig {
//             ip_addresses: None,
//             mac_addresses: None,
//             scopes: None,
